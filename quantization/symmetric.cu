...
int8_t* quantSymCPU(float* input, int8_t* result, int rows, int cols){
  ...
  {
    float MAX = input[j * cols];
    float MIN = input[j * cols];

    for(int i = 0; i < cols; i++)
    {
      MAX = max(abs(input[j * cols + i]), MAX);
      MIN = max(abs(input[j * cols + i]), MIN);
    }
    float scalew = 255 / (MAX - MIN);
    for(int i = 0; i < cols; i++)
    {
      result[j * cols + i] = round(scalew * (input[j * cols + i] - MIN));
    }
  }
...
template<int blockSize>
__device__ float WarpMin(float minV) {
    if (blockSize >= 32 ) minV = min(minV,__shfl_down_sync(0xffffffff,minV,16));
    if (blockSize >= 16 ) minV = min(minV,__shfl_down_sync(0xffffffff,minV,8));
    if (blockSize >= 8 ) minV = min(minV,__shfl_down_sync(0xffffffff,minV,4));
    if (blockSize >= 4 ) minV = min(minV,__shfl_down_sync(0xffffffff,minV,2));
    if (blockSize >= 2 ) minV = min(minV,__shfl_down_sync(0xffffffff,minV,1));
    return minV;
}
...
__global__ void WarpQuant(float* d_in,int8_t* d_out,int rows,int cols) {
    ...
    float minV = d_in[global_warp_id];
    for (int i = global_warp_id;i < global_warp_id + colsPerThread;i++) {
        maxV = max(maxV,d_in[i]);
        minV = max(minV,d_in[i]);
    }
    maxV = WarpMax<colsPerThread>(maxV);
    __syncwarp();

    float scalew = 255 / (maxV - minV);
    for (int i = global_warp_id;i < global_warp_id + colsPerThread;i++) {
        d_out[i] = round(scalew * (d_in[i] - minV));
    }
}
